...
class Login extends CI_Controller {
	...
	public function index()
	{
		...
		{
			$this->session->set_flashdata('info', '<script>swal({title: "Info", text: "You have already logged in", timer: 10000, icon: "info", button: false});</script>');
		...
		if ($this->form_validation->run() == TRUE)
		{
			...
			if ($this->model_login->login($email, $password))
			{
				...
				{
					$session = array('user_id' => $this->model_login->user_id($email));
		...
		{
			$data = array(
				'title' 		=> 'Moneytor Apps &mdash; Monitor Keuanganku',
